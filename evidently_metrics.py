...
#Need to read the data day by day
...
def calculate_metrics_postgresql(curr, i):
    ...
    median_fare_amt = result['metrics'][4]['result']['current']['value']
    rmse = result['metrics'][3]['result']['current']['rmse']

    curr.execute(
        "insert into dummy_metrics(timestamp, prediction_drift, num_drifted_columns, share_missing_values, median_fare_amount, rmse) values (%s, %s, %s, %s, %s, %s)",
        (begin + datetime.timedelta(i), prediction_drift, num_drifted_columns, share_missing_values, median_fare_amt, rmse)
    )
